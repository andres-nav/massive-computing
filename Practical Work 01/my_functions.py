...
def tonumpyarray(mp_arr): # We are pluggin this function here so that we dont need to import it.
    # mp_array is a shared memory array with lock
    return np.frombuffer(mp_arr.get_obj(),dtype=np.uint8)


def initialize_pool1(shared_array_,srcimg, imgfilter, lock):
    ...
    global lock1	      # --> the lock that will use the threads to write in the global memory
...
def filter_image1(row):
    ...
    global my_filter1    # The filter to be applied
    global shared_space1 # The space where to store the resulting value
    global lock1         # The lock to be used when storing the result
    global image_1       # The image to be filtered 
    
    image = image_1
    (rows,cols,depth) = image.shape
    (filter_rows,filter_cols)=my_filter1.shape    
    ...
    if row>0: # The row is not close to the upper border -->  We can use the previous row
        prow=image[row-1,:,:]
    ...
    for d in range(depth):
        ...
        for c in range(cols):
            ...
            if c == 0: # we are in the left border
                ...
                matrix = np.array(( [prevprow[c][d],prevprow[c][d], prevprow[c][d], prevprow[c+1][d],prevprow[c+2][d]],
                                    ...
                                    [nextnrow[c][d],nextnrow[c][d], nextnrow[c][d], nextnrow[c+1][d],nextnrow[c+2][d]]
                                    ))
            
            elif c == 1: # we are in the second col of the left border
                
                matrix = np.array(( [prevprow[c-1][d],prevprow[c-1][d], prevprow[c][d], prevprow[c+1][d],prevprow[c+2][d]],
                                    ...
                                    [nextnrow[c-1][d],nextnrow[c-1][d], nextnrow[c][d], nextnrow[c+1][d],nextnrow[c+2][d]]
                                    ))
                
            elif c == (cols - 2): # we are in the left border
                matrix = np.array(( [prevprow[c-2][d],prevprow[c-1][d], prevprow[c][d], prevprow[c+1][d],prevprow[c+1][d]],
                                    ...
                                    [nextnrow[c-2][d],nextnrow[c-1][d], nextnrow[c][d], nextnrow[c+1][d],nextnrow[c+1][d]]
                                    ))
                
            elif c == (cols - 1): # we are in the previous column of the left border
                matrix = np.array(( [prevprow[c-2][d],prevprow[c-1][d], prevprow[c][d], prevprow[c][d],prevprow[c][d]],
                                    ...
                                    [nextnrow[c-2][d],nextnrow[c-1][d], nextnrow[c][d], nextnrow[c][d],nextnrow[c][d]]
                                    ))
                
            else:# we are not in a border
                matrix = np.array(( [prevprow[c-2][d],prevprow[c-1][d], prevprow[c][d], prevprow[c+1][d],prevprow[c+2][d]],
                                    ...
                                    [nextnrow[c-2][d],nextnrow[c-1][d], nextnrow[c][d], nextnrow[c+2][d],nextnrow[c+2][d]]
                                    ))
...
def image_filter(image, filter_mask, numprocessors, filtered_image):
    ...
    p = mp.Pool(processes=numprocessors, 
                initializer=initialize_pool1,
                initargs=[filtered_image,image,filter_mask, lock])
    p.map(filter_image1, image_rows)
...
def filters_execution(image, filter_mask1, filter_mask2, numprocessors,filtered_image1, filtered_image2):
    ...
    image_filter(image, filter_mask1, numprocessors, filtered_image1)
    image_filter(image, filter_mask2, numprocessors, filtered_image2)
...
def sum_square_with_mp(number):
    ...
    print(f"Processing {len(number)} numbers took {end_time} time using multiprocessing. ")
...
def sum_square_without_mp(number):    
    ...
    end_time = time.time() - start_time
    print(f"Processing {len(number)} numbers took {end_time} time using serial processing. ")
